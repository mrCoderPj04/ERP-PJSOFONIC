...
export default function LoginPage() {
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    setLoadingText('Connecting to EMS...');

    // Progress timer for Render free tier spin-up
    const timer1 = setTimeout(() => {
      setLoadingText('Waking up EMS Cloud instance on Render...');
    }, 3500);

    const timer2 = setTimeout(() => {
      setLoadingText('Render server starting up... Verifying live credentials...');
    }, 8500);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-950 flex items-center justify-center p-4 overflow-y-auto">
      ...
      <div className="w-full max-w-lg bg-gray-900/90 border border-gray-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl relative my-auto">
        {/* Header Branding */}
        <div className="flex flex-col items-center text-center mb-6">
          <div className="w-16 h-16 rounded-2xl bg-gray-900 border border-indigo-500/30 flex items-center justify-center text-white shadow-xl shadow-indigo-600/30 mb-3 p-2">
            <img
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="e.g. EMP-ADM-01 or admin@pjsofonic.com"
          ...
          {errorMessage && (
            <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs flex flex-col gap-2 animate-fadeIn">
              ...
              <button
                type="button"
                onClick={() => handleLogin()}
                className="self-end px-3 py-1 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 rounded-lg text-[11px] font-semibold transition-all"
              >
                Retry Now
              </button>
            </div>
          ...
          {fallbackNotice && (
            <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs flex items-center gap-2">
              ...
              <span className="text-[11px] font-medium">{fallbackNotice}</span>
            </div>
          )}
